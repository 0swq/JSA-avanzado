...
@Component({
  ...
  template: `
    <button [type]="tipo" [disabled]="deshabilitado || cargando"
      class="inline-flex items-center justify-center gap-2 font-medium rounded-lg border-2 transition-all duration-200
             focus:outline-none focus:ring-2 focus:ring-offset-1
             disabled:opacity-50 disabled:cursor-not-allowed select-none bg-transparent"
...
export class BotonContornoComponent {
  ...
  get clases(): string {
    ...
    return `border-blue-600 text-blue-600 hover:bg-blue-50 focus:ring-blue-500 ${tamanios[this.tamanio] ?? 'px-4 py-2 text-sm'} ${this.anchoCompleto ? 'w-full' : ''}`;
  }
}
