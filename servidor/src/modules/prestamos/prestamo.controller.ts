...
export const prestamoControlador = {
  ...
  async obtenerPorUsuario(req: Request, res: Response, next: NextFunction) {
    try {
      const prestamos: RespuestaPrestamoDto[] = await prestamoServicio.obtenerPorUsuario(req.params.usuarioId as string);
      res.json({ success: true, data: prestamos });
    } catch (error) { next(error); }
  },
  ...
  async obtenerPorId(req: Request, res: Response, next: NextFunction) {
    try {
      const prestamo: RespuestaPrestamoDto = await prestamoServicio.obtenerPorId(req.params.id as string);
